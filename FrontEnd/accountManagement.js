    ...
    return cognito
      ...
      .then((auth_info) => {
        ...
        return cognito
          .getUser({
            AccessToken:
              auth_info["AuthenticationResult"]["AccessToken"] /* required */,
          })
          .promise()
          .then((user_info) => {
            user_info_cognito = user_info;
            console.log(user_info_cognito["UserAttributes"]);
            user_credit = user_info_cognito["UserAttributes"].filter(
              (att) => att["Name"] === "custom:credit"
            )[0]["Value"];
